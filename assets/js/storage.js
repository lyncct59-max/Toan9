...
(function (App) {
  ...
  function defaults() {
    return {
      ...
      recordings: {},
      assign: { pin: "", minutesGoal: 30, tasks: [], strict: false }
    };
  }
  ...
  function load() {
    try {
      ...
      if (raw) {
        ...
        const d = defaults();
        for (const k of ["profile", "streak", "lessons", "srs", "notes", "daily", "reminder", "practice", "topics", "think", "tts", "ai", "recordings", "mistakes", "rama", "assign"]) {
  ...
  function setStepDone(id, step) {
    ...
    L.steps[step] = true;
    if (!was) { bumpDaily("steps", 1); bump("step"); }
    save();
  ...
  function assignData() {
    if (!state.assign) state.assign = { pin: "", minutesGoal: 30, tasks: [], strict: false };
    if (typeof state.assign.strict !== "boolean") state.assign.strict = false;
  ...
  function assignSetStrict(on) { assignData().strict = !!on; save(); }
  // Chế độ nghiêm: chỉ mở khu trò chơi khi đủ phút học VÀ xong hết bài được giao
  function strictStatus() {
    const a = assignData();
    if (!a.strict) return { locked: false };
    const goal = a.minutesGoal || 30;
    const mins = minutesToday();
    const tasksLeft = a.tasks.filter(t => !taskDone(t)).length;
    return { locked: mins < goal || tasksLeft > 0, mins: mins, goal: goal, tasksLeft: tasksLeft, tasksTotal: a.tasks.length };
  }
  ...
  function exportState() {
    // không bao giờ để lộ khoá API vào tệp sao lưu
    const copy = JSON.parse(JSON.stringify(state));
    if (copy.ai) copy.ai.key = "";
    return JSON.stringify({ app: "toan9-feynman", schema: SCHEMA, profile: currentProfile().name, t: Date.now(), state: copy }, null, 1);
  }
  ...
  /* ---------- AI Mentor (khoá API do người dùng tự cung cấp) ---------- */
  function aiGet() { if (!state.ai) state.ai = { key: "", model: "gpt-4o-mini" }; return state.ai; }
  function aiSet(patch) { Object.assign(aiGet(), patch || {}); save(); }
  ...
  /* ---------- Thi thử vào 10 ---------- */
  function examsData() { if (!Array.isArray(state.exams)) state.exams = []; return state.exams; }
  function examRecord(deId, score, maxScore, seconds) {
    examsData().push({ deId: deId, score: score, max: maxScore, seconds: seconds, date: dateStr(), t: Date.now() });
    save();
  }
  function examList() { return examsData().slice(); }
  function examBest(deId) {
    let best = null;
    examsData().forEach(e => { if ((!deId || e.deId === deId) && (!best || e.score > best.score)) best = e; });
    return best;
  }
  ...
  /* ---------- Sổ tay lỗi sai (ôn lại theo lịch) ---------- */
  // BOX_DAYS: làm đúng thì lên hộp, giãn dần; sai thì rớt về hộp 0 (hỏi lại sớm)
  const MK_BOX_DAYS = [0, 1, 3];            // hộp 0→hỏi trong ngày, 1→sau 1 ngày, 2→sau 3 ngày
  const MK_GRADUATE = 3;                     // đúng 3 lần (giãn dần) = đã thuộc, gỡ khỏi sổ
  function mistakesAll() { if (!state.mistakes) state.mistakes = {}; return state.mistakes; }
  // Ghi một câu sai. q là object câu hỏi (mc/fill). meta: {source, lessonId, label}
  function mistakeAdd(id, q, meta) {
    if (!id || !q) return;
    const M = mistakesAll();
    const ex = M[id];
    M[id] = {
      q: q,
      source: (meta && meta.source) || (ex && ex.source) || "",
      lessonId: (meta && meta.lessonId) || (ex && ex.lessonId) || "",
      label: (meta && meta.label) || (ex && ex.label) || "",
      box: 0,
      due: dateStr(),
      wrongCount: (ex ? ex.wrongCount : 0) + 1,
      addedAt: ex ? ex.addedAt : Date.now(),
      lastAt: Date.now()
    };
    save();
  }
  // Chấm lại một câu trong sổ. correct=true → lên hộp; sai → về hộp 0.
  // Trả về "graduated" nếu đã thuộc (được gỡ khỏi sổ).
  function mistakeReview(id, correct) {
    const M = mistakesAll();
    ...
    if (correct) {
      m.box = (m.box || 0) + 1;
      if (m.box >= MK_GRADUATE) { delete M[id]; save(); return "graduated"; }
      m.due = addDays(dateStr(), MK_BOX_DAYS[m.box] || 7);
    } else {
      m.box = 0;
      m.due = dateStr();
    ...
    save();
    return M[id] ? "kept" : "graduated";
  }
  function mistakeRemove(id) { const M = mistakesAll(); if (M[id]) { delete M[id]; save(); } }
  function mistakesList() {
    const M = mistakesAll();
    return Object.keys(M).map(id => Object.assign({ id: id }, M[id]));
  }
  function mistakesDueList() {
    const today = dateStr();
    return mistakesList().filter(m => diffDays(m.due, today) >= 0)
      .sort((a, b) => (a.box - b.box) || (a.due < b.due ? -1 : 1));
  }
  function mistakesCount() { return Object.keys(mistakesAll()).length; }
  function mistakesDueCount() { return mistakesDueList().length; }

  /* ---------- Bộ đếm hành động (để nhận biết "việc 5 phút" đã xong) ---------- */
  function bump(k) { if (!state.counters) state.counters = {}; state.counters[k] = (state.counters[k] || 0) + 1; save(); }
  function counter(k) { return (state.counters && state.counters[k]) || 0; }
  function daily5Get() { return state.daily5 && state.daily5.date === dateStr() ? state.daily5 : null; }
  function daily5Set(obj) { state.daily5 = Object.assign({ date: dateStr(), done: false }, obj); save(); return state.daily5; }
  function daily5Done() { if (state.daily5 && !state.daily5.done) { state.daily5.done = true; save(); return true; } return false; }

  /* ---------- Góc Ramanujan ---------- */
  function ramaData() { if (!state.rama) state.rama = {}; return state.rama; }
  function ramaGet(id) { return ramaData()[id] || { guessed: false, done: false }; }
  function ramaGuess(id) { const r = ramaGet(id); r.guessed = true; ramaData()[id] = r; save(); }
  function ramaDone(id) { const r = ramaGet(id); const was = r.done; r.done = true; ramaData()[id] = r; save(); return !was; }
  function ramaDoneCount() { return Object.values(ramaData()).filter(r => r.done).length; }
  ...
  App.Storage = {
    ...
    // sổ tay lỗi sai
    mistakeAdd, mistakeReview, mistakeRemove, mistakesList, mistakesDueList, mistakesCount, mistakesDueCount,
    ramaGet, ramaGuess, ramaDone, ramaDoneCount,
    bump, counter, daily5Get, daily5Set, daily5Done,
    // kiểm tra định kỳ
    testRecord, testList,
    examRecord, examList, examBest, testBestPct,
    // giọng đọc
    ttsGet, ttsSet,
    // AI mentor
    aiGet, aiSet,
    ...
    // giao bài & phụ huynh
    assignGet, assignSetMinutesGoal, assignSetStrict, strictStatus, assignHasPin, assignSetPin, assignClearPin,
